...
public class TalentInsightService {
    ...
    public TalentInsightData analyze(UUID talentId, String keywords) {
        TalentProfile profile = talentProfileRepository.findById(talentId)
                .orElseThrow(() -> new LinkerException(HttpStatus.NOT_FOUND,
                        "TALENT_NOT_FOUND", "?∏Î†• ?ÑÎ°ú?ÑÏùÑ Ï∞æÏùÑ ???ÜÏäµ?àÎã§."));
        ...
        try {
            ...
            throw new LinkerException(HttpStatus.INTERNAL_SERVER_ERROR, "AI_ERROR", "AI Î∂ÑÏÑù ?∏Ï∂ú ?§Ìå®: " + e.getMessage());
        }
    ...
    // AIÍ∞Ä Î∂ÑÎ•ò??projectAssignments + ?§Ï†ú Í≤ΩÎ†• Í∏∞Í∞Ñ?ºÎ°ú ?ÑÎ©î??ÎπÑÏú® Í≥ÑÏÇ∞
    private TalentInsightResponse computeDomainPct(TalentInsightResponse result,
        ...
        // projectName(?åÎ¨∏???ïÍ∑ú?? ??domain Îß§Ìïë
        ...
        // PROJECT ?¥Î†•Î≥??ÑÎ©î??Î∂ÑÎ•ò Î∞?Í∏∞Í∞Ñ ?©ÏÇ∞
        ...
        // pct Í≥ÑÏÇ∞ ???©Í≥ÑÍ∞Ä ?ïÌôï??100???òÎèÑÎ°?Í∞Ä?????ÑÎ©î?∏Ïóê??Ï°∞Ï†ï
    ...
    private String callGemini(String prompt) throws Exception {
        ...
        if (responseBody == null) throw new RuntimeException("Gemini ?ëÎãµ ?ÜÏùå");
    ...
    private TalentInsightResponse parseInsight(String raw, UUID talentId) {
        ...
        if (start < 0 || end <= start) {
            log.error("[INSIGHT_PARSE_ERROR] JSON Î∏îÎ°ù ?ÜÏùå talentId={}", talentId);
            return null;
        }
