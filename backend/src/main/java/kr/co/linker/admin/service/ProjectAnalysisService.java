...
public class ProjectAnalysisService {
    ...
    public ProjectAnalysisResult analyze(String text) {
        ...
        try {
            ...
            log.info("[AI_PROJECT] Î∂ÑÏÑù ?ÑÎ£å: title={}, roles={}", normalized.title(), roles.size());
            return normalized;
        } catch (Exception e) {
            log.error("[AI_PROJECT] Î∂ÑÏÑù ?§Ìå®", e);
    ...
    private String callGemini(String prompt) {
        try {
            ...
            log.debug("[AI_PROJECT] Gemini ?ëÎãµ: {}", clean);
            return clean;
        } catch (Exception e) {
            log.error("[AI_PROJECT] Gemini API ?∏Ï∂ú ?§Ìå®", e);
            return null;
        }
    }
}
