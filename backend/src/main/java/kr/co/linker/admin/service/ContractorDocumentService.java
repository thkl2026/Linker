...
public class ContractorDocumentService {
    ...
    public ContractorDocumentResult analyzeAndUpload(MultipartFile file, String labelName) {
        ...
        // ?§ÌÜ†Î¶¨Ï? ?ÖÎ°ú??        try {
            ...
            log.warn("[CONTRACTOR_DOC] ?åÏùº ?ÖÎ°ú???§Ìå®: {}", e.getMessage());
        ...
        try {
            ...
            if (isImage) {
                ...
                if (text == null || text.isBlank()) {
                    log.warn("[CONTRACTOR_DOC] ?çÏä§??Ï∂îÏ∂ú ?§Ìå®");
                    return new ContractorDocumentResult(null, null, null, null, key, labelName);
                }
            ...
            log.info("[CONTRACTOR_DOC] Î∂ÑÏÑù ?ÑÎ£å key={} regNo={} phone={}", key, registrationNo, phone);
            return new ContractorDocumentResult(registrationNo, phone, bankName, bankAccount, key, labelName);

        } catch (Exception e) {
            log.error("[CONTRACTOR_DOC] AI Î∂ÑÏÑù ?§Ìå®", e);
            return new ContractorDocumentResult(null, null, null, null, key, labelName);
        }
    }
    ...
    private String extractText(byte[] bytes, String lowerName) {
        try {
            ...
            log.warn("[CONTRACTOR_DOC] ?çÏä§??Ï∂îÏ∂ú ?§Ìå®: {}", e.getMessage());
            return null;
        }
    }
    ...
    private String callGeminiVision(byte[] imageBytes, String mimeType) {
        try {
            ...
            String textPart = "??Î¨∏ÏÑú(?¨ÏóÖ?êÎì±Î°ùÏ¶ù ?êÎäî ?µÏû•?¨Î≥∏)?êÏÑú ?¨ÏóÖ?êÎì±Î°ùÎ≤à?? ?ÑÌôîÎ≤àÌò∏, ?Ä?âÎ™Ö, Í≥ÑÏ¢åÎ≤àÌò∏Î•?Ï∂îÏ∂ú?òÏó¨ JSON?ºÎ°úÎß?Î∞òÌôò?òÎùº. " +
                    "?ïÏãù: {\"registrationNo\":null,\"phone\":null,\"bankName\":null,\"bankAccount\":null} ??JSONÎß?Ï∂úÎ†•, ?§Î™Ö Í∏àÏ?.";
            ...
            log.error("[CONTRACTOR_DOC] Gemini Vision ?§Ìå®", e);
            return null;
        }
    }
    ...
    private String callGeminiApi(Map<String, Object> requestBody) {
        try {
            ...
            log.error("[CONTRACTOR_DOC] Gemini API ?§Ìå®", e);
            return null;
        }
    }
